    ...
    if(rst.read()){
        ...
        if(current_step==0){
            if(((!data_valid.read())&&(!layer_id_valid))){
                if(next_load==0)        current_step = 17;
        ...
        if(data_valid_reg.read()){
            ...
            //cout<<"push into fifo: "<< temp_flit <<endl;
            packet_fifo.push(temp_flit);
        }
        else{
            //
        ...
        if(fc8_result.size()==1000){
			...
			for (int i = 0; i < 5; i++) {
                ...
                //std::cout<<indexed_values[i].second<<"\t" <<double(fc8_result[indexed_values[i].second])<<"\t\t"<<(indexed_values[i].first*100)<<"%\t\t"<<class_dict[indexed_values[i].second]<<endl;
			}
        ...
        if(packet_fifo.size()>0){       req_tx.write(1);
        //    cout<<"packet_fifo.front(): "<<packet_fifo.front()<<endl;
        }
        else{                                            req_tx.write(0);
        //            cout<<"packet_fifo.front(): "<<"*******"<<endl;
        }
        if(packet_fifo.size()>0)        flit_tx.write(packet_fifo.front());
        ...
        if(req_rx.read()&& flit_rx.read()[33]==1 )                      ack_rx.write(1);
        else if (ack_rx==1 && req_rx.read()&& flit_rx.read()[32]==1)    ack_rx.write(0);
        else{
            //nothing
        }
