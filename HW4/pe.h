...
void PE::check_packet(Packet* p){
    if(p->is_parameter){
        if(p->is_bias){
            ...
            if(golden_weight.size()!=(this->weights_vec.size())){
                cerr << "[FATAL] In PE_"<< this->id << "# of golden_weight:" << golden_weight.size() << " # of weight:" << this->weights_vec.size() <<endl;
            }
            if(golden_bias.size()!=(this->bias_vec.size())){
                cerr << "[FATAL] In PE_"<< this->id << "# of golden_weight:" << golden_weight.size() << " # of weight:" << this->bias_vec.size() <<endl;
            }

            for(int i=0; i<this->weights_vec.size();i++){
                if(this->weights_vec[i]!=golden_weight[i]){
                    cerr <<"[FATAL] In PE_"<< this->id << ", this->weights_vec[" << i <<"] = " << this->weights_vec[i] << " golden_weight["<<i<<"] = " << golden_weight[i] <<endl;
                }
            }

            for(int i=0; i<this->bias_vec.size();i++){
                if(this->bias_vec[i]!=golden_bias[i]){
                    cerr <<"[FATAL] In PE_"<< this->id << ", this->bias_vec[" << i <<"] = " << this->bias_vec[i] << " golden_weight["<<i<<"] = " << golden_weight[i] <<endl;
                }
            }
